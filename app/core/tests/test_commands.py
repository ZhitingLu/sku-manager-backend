...
class CommandTests(SimpleTestCase):
    ...
    def test_wait_for_db_delay(self, patched_sleep, patched_check):
        ...
        # So the first two times the mocked check() method is called,
        # it will raise a PsycopgError.
        patched_check.side_effect = [PsycopgError] * 2 + \
                                    [OperationalError] * 3 + [True]
        # create a list with three instances of OperationalError.
        # So the next three times the mocked check() method is called,
        # it will raise an OperationalError
        # So after raising OperationalError three times,
        # the next call will return True (the database is ready).
